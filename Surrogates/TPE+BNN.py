...
# Simple Bayesian Neural Network via MC Dropout\ class BNN(nn.Module):
...
if __name__ == '__main__':
    ...
    # Initial samples\    X = np.random.uniform(bounds[0], bounds[1], size=(n_init, 1))
    Y = np.array([black_box(x) for x in X]).reshape(-1, 1)

    # GP surrogate
    gp = GaussianProcessRegressor(kernel=Matern(nu=2.5), alpha=1e-6, normalize_y=True)
    gp.fit(X, Y)

    # Optuna TPE study optimizing PI on GP
    def gp_objective(trial):
        x = trial.suggest_uniform('x', bounds[0], bounds[1])
        lam = schedule_lambda(trial.number+1, budget)
        # use PI as acq: PI = Phi((mu - mu* - xi)/sigma)
    ...
    # Train BNN on initial data\    for epoch in range(2000):
        optimizer.zero_grad()
    ...
    # Evaluate acquisition over grid\    X_grid = np.linspace(bounds[0], bounds[1], 500).reshape(-1,1)
